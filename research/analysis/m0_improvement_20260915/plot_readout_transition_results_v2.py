...
PROTOCOL_SHA = 'deba0647052014de50a00e373582806e6dfa814c16f9c4eaa524d93b66bf77cf'
NAMES = ('CC', 'CO', 'OC', 'OO')
...
def stage(root, mode, complete_sha, policy, inputs):
    ...
    require(summary['sources']['source_sha256'] == manifest['source_sha256'] == policy['source_sha256'] and
            summary['sources']['script_sha256'] == policy['source_sha256']['readout_transition_diagnostic_v2.py'] and
...
def load_inputs(args):
    ...
    require(manifest['schema'] == 'readout-transition-campaign-v1' and manifest['diagnostic_protocol_sha256'] == PROTOCOL_SHA and
            manifest['wrapper_sha256'] == policy['source_sha256']['run_readout_transition_campaign_v2.py'] and
    ...
    producer = module('readout_transition_diagnostic_v2.py', policy, inputs)
    ...
    require(auth['schema'] == 'readout-transition-development-authorization-v1' and auth['status'] == 'FROZEN' and
            ...
            cap <= 1500 and pm['resources'] == dict(max_seconds=300, max_allocated_gib=32), 'Pilot-derived budget/authorization differs')
...
def main(argv=None):
    ...
    parser.add_argument('--diagnostic-protocol', default=str(PKG/'readout_transition_diagnostic_protocol_v2.json'))
    ...
    receipt = dict(schema='readout-transition-results-plot-v2', status='RENDERED_PENDING_VISUAL_QA',
        ...
        engineering_revision=2, previous_plot_script_sha256='9caadb15395832f454df338bf3d712846a659d0b6b9a42aa378d8eebf063e19b',

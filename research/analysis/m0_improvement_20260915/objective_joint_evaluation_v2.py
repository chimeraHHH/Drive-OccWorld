"""V2 native joint evaluation with a source-proven inactive pilot leaf exception.
...
Only historical-cache comparison projects out inactive random sample_traj.
Fresh actual inputs and their complete hashes are never modified.
"""
import argparse
import ast
import copy
...
ADDITIONAL_SOURCE_SHA = {
 'projects/mmdet3d_plugin/datasets/nuscenes_world_dataset_template.py':'9de0dfbba0b77567cf011fb0adb163492e8d5e6f510040b95626b3bcded822cc',
 'projects/mmdet3d_plugin/datasets/samplers/sampler.py':'65b7cea1de9e6563674b6faf80ba809b2497083fe3a5141f92d8ea9e9beee95c',
 'projects/mmdet3d_plugin/models/utils/grid_mask.py':'92589caffcd8102b02cafff78e64614ecf8bc8ad548ab95cbf4d1a898327f150'}
...
def load_contracts(evaluation_path, objective_path, parent_path, verify_runtime=True):
    ...
    require(protocol['schema'] == 'objective-joint-evaluation-protocol-v2' and
    ...
    required = {'objective_joint_evaluation.py','objective_joint_merge.py','joint_native_evaluation.py',
                'merge_joint_evaluation.py','objective_joint_evaluation_v2.py','objective_joint_merge_v2.py',
                'native_boundary_diff.py'}
    require(required <= set(protocol['source_sha256']) and
            protocol['source_sha256']['joint_native_evaluation.py'] == OLD_PRODUCER_SHA and
            protocol['source_sha256']['objective_joint_evaluation.py'] == V1_PRODUCER_SHA and
            protocol['source_sha256']['native_boundary_diff.py'] == DIAGNOSTIC_SOURCE_SHA, 'Missing original/new producer source lock')
    ...
    validate_inactive_policy(protocol,evaluation_path,verify_runtime)
...
def validate_inactive_policy(protocol,evaluation_path,verify_runtime=True):
    policy=protocol['inactive_boundary_policy']
    expected=dict(path='inputs.plan_dict.sample_traj',shape=[1,1800,5,3],dtype='float64',
        planning_must_be_disabled=True,comparison_only_projection=True,
        source_function_ast_sha256=CONSUMER_AST_SHA,previous_cache_source_coverage_claim=False)
    require(set(policy)==set(expected)|{'sources','diagnostic'} and
            all(policy[k]==v for k,v in expected.items()),'Unreviewed inactive-boundary exception')
    sources=policy['sources'];require(len(sources)==len(ADDITIONAL_SOURCE_SHA),'Supplementary source ledger must contain exactly three observed sources')
    for suffix,digest in ADDITIONAL_SOURCE_SHA.items():
        matching=[path for path in sources if Path(path).is_absolute() and str(Path(path)).endswith('/'+suffix)]
        require(len(matching)==1 and sources[matching[0]]==digest,'Wrong supplementary source: '+suffix)
        if verify_runtime:require(sha(matching[0])==digest,'Supplementary runtime source changed: '+suffix)
    evidence=policy['diagnostic'];directory=resolve(evidence['directory'],evaluation_path)
    require(evidence['difference_sha256']==DIFFERENCE_SHA,'Different observed failure cannot authorize this exception')
    done=read(directory/'complete.json',evidence['complete_sha256'])
    difference=read(directory/'difference.json',DIFFERENCE_SHA)
    require(done['status']=='DIFFERENCE_CAPTURED_ORIGINAL_GATE_REJECTED' and
            done['difference_sha256']==DIFFERENCE_SHA and done['original_gate_not_relaxed'] is True and
            done['optimizer_steps']==0,'Diagnostic did not retain the original rejecting gate')
    require(difference['schema']=='native-joint-boundary-difference-v1' and
            difference['diagnostic_source_sha256']==DIAGNOSTIC_SOURCE_SHA and
            difference['original_full_GT_bitwise_equal'] is True and difference['native_full_GT_hist_exact'] is True and
            difference['native_5h_3layer_logits_bitwise_equal'] is True and difference['native_logits_max_abs']==0 and
            difference['native_logits_unequal_elements']==0 and difference['original_gate_not_relaxed'] is True and
            difference['original_forward_and_inputs_unchanged'] is True and difference['planning_enabled'] is False and
            difference['native_module_training_flags']==[False] and difference['optimizer_steps']==0 and
            difference['candidate_performance_computed'] is False,'Missing direct inactive-leaf evidence')
    diffs=difference['differences']
    require(len(diffs)==1 and diffs[0]['path']==expected['path'] and diffs[0]['kind']=='array_values' and
            diffs[0]['shape']==expected['shape'] and diffs[0]['dtype']=='float64','Diagnostic changed a different boundary leaf')
    return policy


def validate_runtime_consumer(native,variants,names):
    from observation_memory import _ast_digest,EXPECTED_SOURCE_FILE_SHA256
    models=dict(M0=native,**variants)
    require(set(models)==set(names),'Incomplete model set for inactive-planning guard')
    result={}
    for name,model in models.items():
        require(model.turn_on_plan is False and all(not m.training for m in model.modules()),
                'Inactive leaf requires planning disabled and every module eval: '+name)
        result[name]=dict(turn_on_plan=False,all_modules_eval=True)
    function=native.future_pred.__func__;source=inspect.getsourcefile(function)
    require(source is not None and sha(source)==EXPECTED_SOURCE_FILE_SHA256,'Original consumer source changed')
    fn=ast.parse(textwrap.dedent(inspect.getsource(function))).body[0]
    require(_ast_digest(fn)==CONSUMER_AST_SHA,'Original future_pred consumer AST changed')
    parents={child:parent for parent in ast.walk(fn) for child in ast.iter_child_nodes(parent)}
    reads=[node for node in ast.walk(fn) if isinstance(node,ast.Subscript) and
           isinstance(node.value,ast.Name) and node.value.id=='plan_dict' and
           isinstance(node.slice,ast.Constant) and node.slice.value=='sample_traj']
    require(len(reads)==1,'Expected exactly one original sample_traj consumer')
    cur=reads[0];guarded=False
    while cur in parents:
        parent=parents[cur]
        if isinstance(parent,ast.If) and cur in parent.body and ast.unparse(parent.test)=="self.turn_on_plan and occ_flow == 'occ'":
            guarded=True
        cur=parent
    require(guarded,'Unique sample_traj consumer is not dominated by the disabled planning branch')
    return result


def compare_cache_boundary(fresh,cached,input_hash,planning_audit,names,tree_digest):
    """Project only a copy used for comparison; preserve both actual trees."""
    import numpy as np
    import torch
    require(set(planning_audit)==set(names) and all(v==dict(turn_on_plan=False,all_modules_eval=True)
            for v in planning_audit.values()),'No runtime proof that planner is inactive')
    fresh_hash=tree_digest(fresh);cache_hash=tree_digest(cached)
    require(fresh_hash==input_hash,'Fresh actual input mutated before cache comparison')
    values=[];projected=[]
    for tree in (fresh,cached):
        require(type(tree) is dict and type(tree.get('plan_dict')) is dict and 'sample_traj' in tree['plan_dict'],
                'Inactive exception requires the original exact dictionary path')
        field=tree['plan_dict']['sample_traj']
        require(torch.is_tensor(field) or isinstance(field,np.ndarray),'Unsupported inactive leaf type')
        array=field.detach().cpu().numpy() if torch.is_tensor(field) else field
        require(array.dtype==np.float64 and list(array.shape)==[1,1800,5,3] and
                bool(np.isfinite(array).all()),'Inactive field dtype/shape/finite contract changed')
        values.append(array)
        plan=dict(tree['plan_dict']);del plan['sample_traj']
        projection=dict(tree);projection['plan_dict']=plan
        projected.append(tree_digest(projection))
    require(projected[0]==projected[1],'Active typed boundary differs from frozen pilot cache')
    require(type(fresh['plan_dict']['sample_traj']) is type(cached['plan_dict']['sample_traj']),
            'Inactive leaf Python type differs from cache')
    x,y=values;delta=np.abs(x-y)
    audit=dict(path='inputs.plan_dict.sample_traj',comparison_only_projection=True,
        actual_inputs_modified=False,fresh_full_input_sha256=fresh_hash,cache_full_input_sha256=cache_hash,
        fresh_projected_input_sha256=projected[0],cache_projected_input_sha256=projected[1],
        fresh_field_sha256=tree_digest(fresh['plan_dict']['sample_traj']),
        cache_field_sha256=tree_digest(cached['plan_dict']['sample_traj']),
        fresh_shape=list(x.shape),cache_shape=list(y.shape),fresh_dtype=str(x.dtype),cache_dtype=str(y.dtype),
        fresh_type=type(fresh['plan_dict']['sample_traj']).__module__+'.'+type(fresh['plan_dict']['sample_traj']).__qualname__,
        cache_type=type(cached['plan_dict']['sample_traj']).__module__+'.'+type(cached['plan_dict']['sample_traj']).__qualname__,
        elements=int(x.size),unequal_count=int(np.count_nonzero(x!=y)),
        max_absolute_difference=float(delta.max()),mean_absolute_difference=float(delta.mean()),
        field_bitwise_equal=x.tobytes()==y.tobytes(),planning_disabled_all_models=planning_audit)
    require(tree_digest(fresh)==fresh_hash and tree_digest(cached)==cache_hash,'Comparison projection mutated an actual input')
    return dict(full_boundary_hash_equal=fresh_hash==cache_hash,active_boundary_hash_equal=True,
                inactive_random_sample_traj_audit=audit)
...
def full_gate(path, evaluation_path, protocol, names):
    ...
    require(complete['schema'] == 'objective-joint-merge-complete-v2' and
            complete['status'] == 'COMPLETE' and complete['mode'] == 'pilot' and complete['samples'] == 2 and
    ...
    require(summary['schema'] == 'objective-joint-pilot-merge-v2' and
            ...
            summary['merger_sha256'] == protocol['source_sha256']['objective_joint_merge_v2.py'] and
            summary['inactive_boundary_policy'] == protocol['inactive_boundary_policy'], 'Pilot did not prove the selected joint implementation')
...
def derive_run(base, namespace):
    ...
    original = copy.deepcopy(node); counts = dict(common=0,mode=0,sample_schema=0,pilot_flag=0,cap=0,head_gate=0,pilot_ledger=0,cache_boundary=0,parity_truth=0)
    ...
    class Adapt(ast.NodeTransformer):
        def visit_Assign(self,item):
            if len(item.targets) == 1 and isinstance(item.targets[0],ast.Name):
                ...
                if target == 'wrapped':
                    counts['head_gate'] += 1
                    new=ast.parse('self.check_loaded_heads(head_digests,migrations,arm_receipts,native,variants)').body[0]
        ...
        def visit_Constant(self,item):
            if item.value == 'native-joint-evaluation-sample-v1':
                counts['sample_schema'] += 1;new=ast.Constant('objective-joint-sample-v2')
                replacements[id(new)]=copy.deepcopy(item);return new
        ...
        def visit_Expr(self,item):
            call = item.value
            if isinstance(call,ast.Call) and isinstance(call.func,ast.Name) and call.func.id=='require' and any(
                    isinstance(x,ast.Constant) and x.value=='Fresh observed boundary differs from frozen pilot cache' for x in call.args):
                counts['cache_boundary']+=1
                new=ast.parse("cache_boundary_parity=self.compare_cache_boundary(context['inputs'],old_inputs,input_hash)").body[0]
                replacements[id(new)]=copy.deepcopy(item);return new
            if isinstance(call,ast.Call) and isinstance(call.func,ast.Attribute) and isinstance(call.func.value,ast.Name) and call.func.value.id=='parity' and call.func.attr=='update' and any(k.arg=='full_boundary_hash_equal' for k in call.keywords):
                counts['parity_truth']+=1
                new=ast.parse('parity.update(cache_boundary_parity,exact_gt_equal=True,native_all_5h_3layer_logits_bitwise=True)').body[0]
                replacements[id(new)]=copy.deepcopy(item);return new
        ...
        def visit_Call(self,item):
            ...
            if isinstance(item.func,ast.Name) and item.func.id == 'dict' and any(
                ...
                item.keywords.append(ast.keyword(arg='schema',value=ast.Constant('objective-joint-shard-complete-v2')))
                item.keywords.append(ast.keyword(arg='planning_disabled_all_models',value=ast.parse('self.planning_audit',mode='eval').body))
            return item
    ...
    require(ast_sha(restored)==ast_sha(original),'Inverse of nine audited edits does not restore original native run')
    node=derived
    receipt = dict(original_producer_sha256=OLD_PRODUCER_SHA,original_run_ast_sha256=ast_sha(original),
        ...
        pilot_cache_comparison='all typed leaves exact except source-proven inactive inputs.plan_dict.sample_traj',
        complete_fresh_and_cache_hashes_retained=True,actual_inputs_never_projected_or_replaced=True,
        original_GT_M0_logits_and_each_model_dev_confusion_gates_unchanged=True,
        planning_disabled_runtime_assertion_all_models=True)
...
class ObjectiveEvaluation:
    ...
    def common_contract(self,parent,selection,receipts):
        return dict(schema='objective-joint-evaluation-contract-v2',mode=self.a.mode,models=self.names,
            ...
            inactive_boundary_policy=self.protocol['inactive_boundary_policy'],
            resources=self.effective_resources,allocator_cap_bytes=int(self.effective_resources['max_allocated_gib']*2**30))

    def check_loaded_heads(self,heads,migrations,receipts,native,variants):
        self.planning_audit=validate_runtime_consumer(native,variants,self.names)
    ...
    def compare_cache_boundary(self,fresh,cached,input_hash):
        return compare_cache_boundary(fresh,cached,input_hash,self.planning_audit,self.names,self.base.tree_digest)

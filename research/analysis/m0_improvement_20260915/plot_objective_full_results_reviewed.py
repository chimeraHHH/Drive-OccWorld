...
def write_numbers(out, summary, names):
    ...
    fields = ['left','right','horizon_seconds','gmo_delta_ratio','gmo_delta_pp','gmo_CI95_lower_pp','gmo_CI95_upper_pp','delta_FP','delta_FN']
    with (out/'horizon_contrast_numbers.csv').open('x',newline='') as stream:
        writer = csv.DictWriter(stream,fieldnames=fields); writer.writeheader()
        for contrast in summary['comparisons']:
            for row in contrast['horizons']:
                writer.writerow(dict(left=contrast['left'],right=contrast['right'],
                    horizon_seconds=row['horizon_seconds'],gmo_delta_ratio=row['gmo_delta_ratio'],gmo_delta_pp=row['gmo_delta_pp'],
                    gmo_CI95_lower_pp=row['gmo_ci95_pp'][0],gmo_CI95_upper_pp=row['gmo_ci95_pp'][1],
                    delta_FP=row['delta_FP'],delta_FN=row['delta_FN']))
...
def main(argv=None):
    ...
    caption += ('Current-frame (t0) values are retained in horizon_numbers.csv and excluded from the future macro metric: '+
        '; '.join(t0_rows)+'. The figure footer displays point differences; all five horizon contrasts and their paired 95% intervals, including t0, are retained in horizon_contrast_numbers.csv and were independently recomputed from the full integer confusions. '
    ...
    filenames = ['objective_full_results.pdf','objective_full_results.png','objective_full_results.svg',
                 'horizon_numbers.csv','contrast_numbers.csv','horizon_contrast_numbers.csv','caption.txt']
    ...
    receipt = dict(schema='objective-full-results-plot-v3',status='RENDERED_PENDING_VISUAL_QA',
        ...
        current_frame_paired_interval_computed=True,current_frame_paired_interval_plotted=False,
        render_revision_reason="Correct prior reporting flag and export already-recomputed paired horizon intervals; no metric or visual curve changes",
        historical_cache_projection_applied=False,

...
job=root/'jobs/readout_transition_v4'; campaign=root/'campaign_readout_transition_v4'
...
value=dict(checked_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),host=os.uname().nodename,
 job='readout_transition_v4',runner_state=state,campaign_state=cs,processes=[process(x) for x in expected],gpu=gpu,
...
if __name__ == '__main__':
    ...
    output=BASE/'receipts'/('readout_transition_v4_monitor_'+datetime.datetime.now().strftime('%Y%m%d_%H%M%S_%f')+'.json')

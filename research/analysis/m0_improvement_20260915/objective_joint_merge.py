"""Hash-checked merging of fixed objective-model native evaluation shards.
...
def validate_sample(result, identity, contract_sha, mode, models):
    require(result['schema'] == 'objective-joint-sample-v1' and result['contract_sha256'] == contract_sha and
    ...
    if mode == 'pilot':
        flags = ('full_boundary_hash_equal','exact_gt_equal','native_all_5h_3layer_logits_bitwise',
...
def load_protocols(a):
    ...
    require(evaluation['schema'] == 'objective-joint-evaluation-protocol-v1' and
    ...
    require(evaluation['source_sha256']['objective_joint_merge.py'] == sha(__file__), 'Merger revision differs from protocol')
    source_union = {}
    ...
    producer = frozen_module('objective_joint_evaluation.py',evaluation['source_sha256']['objective_joint_evaluation.py'])
    ...
    return dict(parent=parent,objective=objective,evaluation=evaluation,evaluation_sha=evaluation_sha,
        ...
        expected_recipe=expected_recipe,nomination_evidence=nomination_evidence)
...
def full_authorization(a, context):
    ...
    require(summary['schema'] == 'objective-joint-pilot-merge-v1' and summary['status'] == 'PILOT_ENGINEERING_PARITY_RESOURCE_PASS' and
...
def collect(a, context):
    ...
    require(contract['schema'] == 'objective-joint-evaluation-contract-v1' and contract['mode'] == a.mode and
            contract['models'] == list(names) and contract['horizons'] == HORIZONS, 'Common model/scope contract differs')
    require(contract['script_sha256'] == evaluation['source_sha256']['objective_joint_evaluation.py'] and
    ...
    for shard_index in range(2):
        ...
        require(done['status'] == 'COMPLETE_JOINT_NATIVE_EVALUATION_SHARD' and done['mode'] == a.mode and
        ...
        shards.append(dict(shard_index=shard_index,samples=len(expected),seconds=done['seconds'],complete_sha256=done_sha,
            ...
            pilot_reference_sources_sha256=pilot_sources_sha))
...
def main(argv=None):
    ...
    sources = dict(evaluation_protocol_sha256=context['evaluation_sha'],objective_train_protocol_sha256=OBJECTIVE_SHA,
        ...
        producer_sha256=evaluation['source_sha256']['objective_joint_evaluation.py'],
        ...
        all_local_bound_python_sources_rehashed=True)
    ...
    if a.mode == 'pilot':
        summary = dict(common,schema='objective-joint-pilot-merge-v1',status='PILOT_ENGINEERING_PARITY_RESOURCE_PASS',
            parity=dict(all_sample_hash_chains_valid=True,full_official_pilot_order=True,
                all_models_five_horizon_GT_counts_match=True,all_native_boundary_GT_logits_and_reference_confusion_checks_passed=True,
            ...
            interpretation='Two frozen scenes establish engineering parity/resources only; no effectiveness score or full-run timing guarantee')
    else:
        ...
        summary = dict(common,schema='objective-joint-full-summary-v1',status='COMPLETE_FULL_NATIVE_COMPARISON',
    ...
    if a.mode == 'pilot':
        (out/'report.md').write_text('# 原生共同评价工程预检\n\n两条冻结 validation anchors、两个不同场景、两个互斥分片的完整哈希链、实际模型权重、五时域 GT 计数及既有参考 parity 全部通过。模型为 '+', '.join(names)+'。本文件只报告工程与资源，不计算或汇报性能提升；两条首样本时延不能保证完整验证耗时。所有五模型开发结果与固定晋级依据见 development_disclosure.json。\n')
    else:
    ...
    write(out/'complete.json',dict(status='COMPLETE',mode=a.mode,samples=len(ordered),scenes=summary['scenes'],

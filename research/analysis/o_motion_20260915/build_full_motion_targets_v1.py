"""CPU-only full5119 raw nuScenes box chains, separate from all model/cache inputs.
...
def main(argv=None):
    ...
    require(selection['schema'] == 'native-joint-validation-selection-v1' and selection['scope'] == 'full_native_validation', 'Wrong full selection schema')
    require(len(records) == 5119 and len({r['sample_token'] for r in records}) == 5119, 'Require all5119 unique anchors')
    require(Counter(r['split'] for r in records) == {'validation': 5119}, 'Wrong full validation count')
    require(selection['samples'] == 5119 and selection['scenes'] == 150, 'Wrong full selection size')
    ...
    try:
        ...
        require(len(scene_sets['validation']) == 150, 'Full validation must contain all150 scenes')
        lidar = {}
        ...
        manifest=dict(schema='raw-nuscenes-motion-target-manifest-v1', status='COMPLETE_RAW_LABEL_EXTRACTION',
            ...
            samples=5119,scenes=150,scope='full_native_validation',unique_frame_samples=len(wanted),all_official_scene_memberships_checked=True,
        ...
        write_json(out/'complete.json',dict(schema='raw-nuscenes-motion-target-complete-v1',status='COMPLETE',
            samples=5119,validation_samples=5119,scenes=150,script_sha256=script_sha,
            selection_sha256=SELECTION_SHA,manifest_sha256=sha(out/'manifest.json'),statistics_sha256=sha(out/'statistics.json'),
            seconds=time.monotonic()-started,all5119_files_roundtrip_verified=True,model_or_prediction_read=False,optimizer_steps=0))
        signal.alarm(0)

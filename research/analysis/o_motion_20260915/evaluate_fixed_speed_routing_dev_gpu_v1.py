"""Bounded original-GPU evaluation of all eight fixed speed routes on original dev200.
...
STATUS = 'COMPLETE_GPU_FIXED_SPEED_ROUTING_DEV_DIAGNOSTIC'
HORIZONS = (.5, 1., 1.5, 2.)
...
GPU_PREFLIGHT = dict(physical_index=0, uuid=GPU_UUID, idle_used_MiB=0, idle_utilization_percent=0, compute_processes=0)
...
def gpu_idle_preflight():
    """Last worker-side observation before CUDA setup; dispatcher owns scheduling."""
    def query(arguments):
        return subprocess.run(['nvidia-smi']+arguments, check=True, capture_output=True,
                              text=True, timeout=10).stdout.strip()
    gpu_text = query(['--query-gpu=index,uuid,memory.used,utilization.gpu', '--format=csv,noheader,nounits'])
    rows = [[part.strip() for part in line.split(',')] for line in gpu_text.splitlines()]
    selected = [row for row in rows if row[1] == GPU_UUID]
    require(len(selected) == 1 and int(selected[0][0]) == 0, 'Physical GPU 0 UUID binding differs')
    require(int(selected[0][2]) == GPU_PREFLIGHT['idle_used_MiB'] and
            int(selected[0][3]) == GPU_PREFLIGHT['idle_utilization_percent'], 'Scheduled GPU memory/utilization is not zero')
    ...
    require(not any(row[0] == GPU_UUID for row in processes), 'Scheduled GPU already has compute processes')
    return dict(physical_index=0, uuid=GPU_UUID, used_MiB=int(selected[0][2]),
                utilization_percent=int(selected[0][3]), compute_processes=0,
                scope='last pre-CUDA observation; scheduling exclusivity is managed by dispatcher')
...
def run(a, out, started, progress):
    ...
    idle_receipt = gpu_idle_preflight()
    import torch
    ...
    manifest = dict(schema=SCHEMA, source_sha256=sha(__file__), protocol_sha256=sha(a.protocol), protocol=p,
        ...
        sparse_label_manifest_sha256=connected['labels']['manifest_sha256'], runtime=runtime, gpu_preflight=idle_receipt,
        GPU_reference_complete_sha256=GPU_COMPLETE_SHA, GPU_reference_objects_sha256=GPU_OBJECTS_SHA,

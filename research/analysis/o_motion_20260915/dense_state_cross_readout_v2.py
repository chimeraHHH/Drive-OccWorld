"""Fixed-final cross-readout v2: deterministic inference, unchanged parity tolerance.

v1 failed strict reconstructed-logit parity before completing a sample.
This revision changes only the audit inference numerical policy; original
checkpoints, model/transport sources, population and metrics remain fixed.
"""
import argparse
import hashlib
import json
...
def main(a):
        ...
        with torch.no_grad():
            ...
            for c,f in combinations:
                ...
                if (c==f and c in ['P','J']) or (c=='D' and f=='zero'):
                    ...
                    assert torch.allclose(logits,reference,atol=1e-5,rtol=1e-5),(index,key,error)
                ...
                if c in ['P','J','D'] and f=='zero':
                    ...
                    assert torch.allclose(logits,zero_predictions[c],atol=1e-5,rtol=1e-5),(index,key,error)
    ...
    result=dict(status='COMPLETE_FIXED_TRAIN4_CROSS_READOUT',scope='fixed final512 train4 representation/readout attribution, not held-out or independent causal effects',
        ...
        numerical_policy=dict(deterministic_algorithms=True,cudnn_deterministic=True,parity_tolerance_unchanged=True),

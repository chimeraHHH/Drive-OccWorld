...
ARMS = ('zero', 'D', 'CRN_CV', 'CV_D_speed05', 'surface_all_zero',
        'surface_quality_zero', 'CV_surface_all_uncovered', 'CV_surface_quality_uncovered')
...
def main(a):
    ...
    assert done['status']=='COMPLETE_FROZEN_HISTORY_TRACKER' and done['optimizer_updates']==0 and not done['GT_read']
    ...
    rule={'status':'FIXED_RULES_NO_FIT','source_sha256':sha(__file__),'surface_complete_sha256':sha(a.surface/'complete.json'),
          ...
          'radius_m':1.,'surface_variants':['in_roi','quality_in_roi'],'quality_definition':protocol['quality_mask'],'arms':list(ARMS),
    ...
    for identity in protocol['anchors']:
        ...
        clouds={key:[[],[]] for key in ('in_roi','quality_in_roi')}
        ...
        for hi,horizon in enumerate(HORIZONS):
            ...
            for clock,dt in [('nominal',horizon),('actual',float(v['dt_future_seconds'][hi]))]:
                ...
                all_v=queried['in_roi'][0]*dt;fb_v=queried['quality_in_roi'][0]*dt
                ...
                for arm,pred in zip(ARMS,predictions):
                    ...
                    for oi in np.flatnonzero(valid):
                        object_rows.append(dict(sample_token=identity['sample_token'],scene_token=identity['scene_token'],
                            ...
                            all_support_fraction=float(cover['in_roi'][oi]),quality_support_fraction=float(cover['quality_in_roi'][oi])))
    rows=[]
                ...
                for arm in ARMS:
                    ...
                    rows.append(dict(clock=clock,horizon_s=h,group=group,arm=arm,objects=len(selected),
                        ...
                        **{name:float(np.mean([r[name] for r in selected])) for name in
                           ('xy_epe_m','xyz_epe_m','predicted_speed_mps','all_support_fraction','quality_support_fraction')}))
    ...
    result={'status':'COMPLETE_FULL_ORIGINAL_POPULATION_TRACKER_TRAIN16_DIAGNOSTIC','anchors':len(audits),

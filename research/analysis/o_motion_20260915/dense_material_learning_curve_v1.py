"""Matched fixed-material versus evolving-content train4 diagnostic.

All recurrent capacity, initial parameters, losses, ordering and optimizer
are preserved. Only the source content read by transport differs. No dev.
"""
import argparse
import hashlib
import importlib
...
def run(a):
    ...
    assert p['schema'] == 'dense-material-learning-curve-v1'
    ...
    from dense_material_state_v1 import DenseMaterialState, mechanism_checks
    ...
    write(out/'mechanism_checks.json', mechanism_checks())
    ...
    def make_model(arm):
        ...
        m = DenseMaterialState(material_mode='fixed' if arm=='T' else 'evolving').cuda(); m.load_state_dict(shared, strict=True)
        for module in (m.encoder, m.decoder):
            for parameter in module.parameters():
                parameter.requires_grad_(False)
        return m

    init = make_model('J'); dataset = []
    for i in selected:
        check(); r = records[i]
    ...
    write(out/'manifest.json',manifest);summary=dict(scope='train4 fixed rendered material versus evolving content; not generalization or O comparison',arms={})
    for arm in ['T','J']:
        model=make_model(arm); params=[v for v in model.parameters() if v.requires_grad]
        ...
        if arm=='T':
            with torch.no_grad():
                z=model(dataset[0]['tokens'].cuda(),'transport','zero')['logits']
                assert torch.equal(z,z[:,:1].expand_as(z))
        ...
        for step in range(1,p['updates_per_arm']+1):
            ...
            occ=occ_loss(forecast['logits'][0],d['coarse'].cuda())
            loss=p['physical_weight']*physical+occ
    ...
    reference=np.asarray(summary['arms']['T'][0]['occupancy']['normal']['hist']).sum(2)
    t0=np.asarray(summary['arms']['T'][0]['occupancy']['normal']['hist'])[0]
    ...
    files=['mechanism_checks.json','protocol.json','manifest.json','summary.json','training.jsonl','milestones.jsonl','objects.jsonl']
    files += [path.name for path in out.glob('*.pth')]
    write(out/'complete.json',dict(status='COMPLETE_TRAIN4_MATERIAL_LEARNING_CURVES',protocol_sha256=a.protocol_sha256,
        updates=2*p['updates_per_arm'],development_read=0,files_sha256={name:sha(out/name) for name in files}))

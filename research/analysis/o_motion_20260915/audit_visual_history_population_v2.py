...
Raw sample.json has no devkit-generated data reverse index; camera file
availability is explicitly outside this metadata-only audit.
"""
import argparse
import hashlib
import json
...
def run(a):
    ...
    evidence = dict(schema='visual-history-population-audit-v2',
        ...
        limitation='Metadata availability only; sample-chain availability only, no camera association check; sample timestamps are not acquisition times; H includes current; sensor files not decoded/validated')
    details = []

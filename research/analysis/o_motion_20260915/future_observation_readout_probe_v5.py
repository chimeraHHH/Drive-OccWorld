...
def observed_state(dataset, model, index, device):
    ...
    for earlier, later in zip(rows[:-1], rows[1:]):
        assert dataset.nusc.get('sample', earlier['token'])['next'] == later['token'], 'Nonconsecutive sensor sample chain'
    ...
    # Native CustomCollect3D emits missing requested fields as None.
    assert all(e.get('gt_occ') is None and e.get('segmentation') is None for e in examples)
    examples = [{key: e[key] for key in ('img', 'img_metas')} for e in examples]
...
def main():
    ...
    assert sha(dataset_cfg.ann_file) == index['ann_sha256']
    ...
    for ordinal, row in enumerate(rows):
        ...
        for h in range(5):
            ...
            event('OBSERVATION_ENCODED', ordinal=ordinal, horizon=h)
            ...
            if h == 0:
                assert np.allclose(transform, np.eye(4), rtol=0, atol=1e-8)
                identity_resampling_max_abs = (warped-direct).abs().max().item()
                # Identity grid_sample roundoff depends on local logit gradients.
                # This unused value is recorded; the actual t0 output is exact O.
            ...
            # Original protocol uses consecutive frame slots; real sampling is irregular.
            assert observation['actual_dt_seconds'] == 0 if h == 0 else observation['actual_dt_seconds'] > observations[-1]['actual_dt_seconds']
            if h > 0:
                assert dataset.nusc.get('sample', observations[-1]['sample_tokens'][-1])['next'] == observation['sample_tokens'][-1]
        ...
        record = dict(ordinal=ordinal, sample_token=row['sample_token'], scene_token=row['scene_token'],
            ...
            unused_identity_resampling_max_abs=identity_resampling_max_abs,

...
Operational amendments only: shared GPU0 admission, 12 GiB allocator cap, 7200-second limit.
"""
import argparse
import csv
import io
...
OPERATIONAL_AMENDMENT = {'kind': 'shared_gpu_admission_allocator_cap_and_runtime_budget_only',
 ...
 'runtime_amendment': {'previous_shared_protocol_sha256': '64ddda82da8f2e1726cb827b1b24b3209fd8dc0f880d99b0f757a8a7bebf3681',
                       'previous_shared_collector_sha256': '55097e1bb16752cd591e7cf0b71fa1389be2f01466347854ed4a2a1ce6784f6c',
                       'previous_max_seconds': 3600,
                       'max_seconds': 7200,
                       'outer_runner_seconds': 7260,
                       'reason': 'Observed shared-GPU anchors 2 through 6 take approximately 8.2 '
                                 'seconds per anchor; estimated 512-anchor duration approximately 70 '
                                 'minutes exceeds the previous 3600-second cap. Resource observation '
                                 'only, not scientific scores.',
                       'scientific_computation_unchanged': True,
                       'fresh_output_required': True,
                       'resume_previous_partial_output': False}}
...
def check_shared_gpu(uuid):
    """Observe the fixed GPU0; other users are allowed, never signalled."""
    contract = OPERATIONAL_AMENDMENT
    require(uuid == contract['gpu_uuid'] and os.environ.get('CUDA_VISIBLE_DEVICES') == uuid,
            'Expose exactly the fixed physical GPU0 UUID')
    def query(arguments):
    ...
    require(len(rows) == 1 and len(rows[0]) == 7, 'Ambiguous shared GPU snapshot')
    fields = [x.strip() for x in rows[0]]
    require(fields[0] == '0' and fields[1] == uuid, 'Physical GPU0 identity differs')
    total, used, free, utilization = [int(x) for x in fields[3:]]
    require(total > 0 and 0 <= used <= total and 0 <= free <= total and 0 <= utilization <= 100,
            'Invalid GPU memory/utilization snapshot')
    require(free * 1024**2 >= contract['minimum_free_bytes'], 'Shared GPU has less than 16 GiB free')
    apps = query(['--query-compute-apps=gpu_uuid,pid', '--format=csv,noheader,nounits'])
    pids = []
    for row in csv.reader(io.StringIO(apps)):
        fields_app = [x.strip() for x in row]
        require(len(fields_app) == 2 and fields_app[0].startswith('GPU-'), 'Invalid compute-app snapshot')
        pid = int(fields_app[1]); require(pid > 0, 'Invalid compute PID')
        if fields_app[0] == uuid:
            pids.append(pid)
    return dict(index=0, uuid=uuid, name=fields[2],
        checked_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),
        prelaunch_memory_total_MiB=total, prelaunch_memory_used_MiB=used,
        prelaunch_memory_free_MiB=free, prelaunch_utilization_percent=utilization,
        existing_compute_pids=pids, shared_execution=True,
        minimum_free_bytes=contract['minimum_free_bytes'],
        admission_is_observation_not_memory_reservation=True)
...
def load_model(a, probe):
    ...
    gpu = check_shared_gpu(a.gpu_uuid)
    require(torch.cuda.is_available() and torch.cuda.device_count() == 1, 'Exactly one CUDA GPU required')
    # Set the caching-allocator limit before moving any model/image to CUDA.
    # The fraction is of total device memory, not currently free memory.
    torch.cuda.set_device(0)
    total_bytes = torch.cuda.get_device_properties(0).total_memory
    cap_bytes = OPERATIONAL_AMENDMENT['allocator_cap_bytes']
    require(total_bytes >= OPERATIONAL_AMENDMENT['minimum_free_bytes'], 'Unexpected device capacity')
    fraction = cap_bytes / total_bytes
    torch.cuda.set_per_process_memory_fraction(fraction, 0)
    gpu.update(allocator_cap_bytes=cap_bytes, allocator_total_memory_bytes=total_bytes,
               allocator_fraction=fraction, allocator_cap_installed_before_model_to_cuda=True)
...
def run(a, out, started, progress):
    ...
    require(protocol['schema'] == SCHEMA and protocol['status'] == 'FROZEN' and
            protocol['resources'] == dict(max_seconds=7200, cpu_threads=2, max_allocated_bytes=12*1024**3) and
    ...
    require(protocol.get('operational_amendment') == OPERATIONAL_AMENDMENT, 'Shared operational amendment changed')
    ...
    manifest = dict(schema=SCHEMA, source_sha256=sha(__file__), protocol_sha256=sha(a.protocol),
        ...
        operational_amendment=protocol['operational_amendment'],
    ...
    for ordinal, (row, old) in enumerate(zip(rows, camera_rows)):
        ...
        require(time.monotonic()-started < 7200, 'Frozen batch time bound exceeded')
    ...
    require(elapsed < 7200, 'Extraction exceeded frozen time budget')
...
def main():
    ...
    signal.alarm(7200)
    try:
        require(a.max_seconds == 7200 and os.environ.get('CUDA_VISIBLE_DEVICES') == a.gpu_uuid, 'Fixed resource settings changed')

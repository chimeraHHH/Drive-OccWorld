...
SCHEMA = 'common-connected-motion-evaluation-v2'
ARMS = ('O', 'J', 'D')
PINNED = {
...
def check_sources(protocol):
    ...
    require('train_connected_motion_v2.py' in bindings, 'Protocol must bind actual J/D trainer')
...
def run(a, out, started, stopping):
    require(sha(a.protocol) == a.protocol_sha256 == TRAINING_PROTOCOL_SHA, 'Actual frozen V2 protocol bytes differ')
    p = read(a.protocol)
    require(p['status'] == 'FROZEN' and p['schema'] == 'connected-motion-training-v2', 'Frozen J/D protocol required')
    ...
    trainer = import_bound('train_connected_motion_v2', sources)

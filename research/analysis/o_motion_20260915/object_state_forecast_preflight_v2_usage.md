# V/G 原生四次更新预检 v2（待实际资产与冻结，未派发）

入口是 `object_state_forecast_preflight_v2.py`。它只允许既有 seed11 第一轮排列的前 16 个 train512 样本、每臂 4 次 accum4 更新，不保存权重、不读开发集、不产生成绩。原 K/B 预检、V/G v1、O、D 和 physical readout 保留。v2 conditioner 的外部27维不变，内部追加仅由预测状态推导的 p0+h*v 三维假设，G 在推导之前清零速度；参数95,872。它不是硬空间遮罩或直接物理输出捷径。

`object_state_forecast_preflight_protocol_v2.draft.json` 的状态为 `REVIEW_REQUIRED`。其中三个尚不存在的 train512 预测资产 SHA 保留 null；实际 CPU/GPU 数据准备完成并审核后，由根任务填写真实 complete/manifest/predictions SHA、核验推理源 SHA、确定预算并冻结状态。没有自动 pilot→正式训练。草案资源 600 秒/32 GiB 只是沿用预检上限，尚无本轮实测。
...
本地 `object_state_forecast_preflight_v2_cpu_checks.json` 记录 Python3.10 语法、CLI、NumPy 坐标/类别/空集合检查、实际 dev200 资产拒绝，以及真实首16训练元数据的身份/姿态/SHA。没有运行本轮 Torch/native 前向、backward 或 optimizer；v2 conditioner 的10项实际 H200 Torch CPU测试（object_state_conditioner_cpu_v2/receipt.json）是独立既有证据，不能代替本次四更新预检。新 v2 CPU回执绑定并复用 v1 的实际首16姿态检查，同时核对未改变的训练/RNG/干预函数 AST。

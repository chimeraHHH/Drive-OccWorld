"""CPU-only CRN predicted-state CV on the unchanged dev200 sparse support.
...
def run(args):
    ...
    result.update(schema='predicted-object-state-cv-diagnostic-v1', status='COMPLETE_READ_ONLY_DIAGNOSTIC',
...
def main():
    ...
    complete = dict(schema='predicted-object-state-cv-complete-v1', status=result['status'], samples=200, scenes=100,

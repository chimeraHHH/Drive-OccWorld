"""Draft fixed-final V/G native state-conditioned forecast training.

Requires an actual successful source-matched discarded preflight and frozen
formal budget. Only predicted velocity input differs. Whole future heads and
new conditioner/readout train with original O occupancy plus physical losses.
No resume, best checkpoint or development evaluation before final update512.
"""
import argparse
import copy
import gzip
...
PREFLIGHT_SOURCE_SHA = '6160b6741f525abcebfccc2b3549b21abffc75b2263bfa5c31c3c8a5231eff9b'
DEV_INPUT_SOURCE_SHA = '7cd498d13d45d5960c6f494ebf7b750718b397d90720ecd745a8c527b0dace06'
...
TRAINING.update(updates=512,examples=2048,passes=4,formal_authorized=True,weights_persisted=True,
    ...
    velocity_intervention_after_updates=None,velocity_intervention_ordinal=None,
    diagnostic_dual_loss_VJP_microsteps=[12,2044],diagnostic_VJP_controls_training=False,
    empty_state_optimizer='original AdamW skips parameters with None gradient; record actual parameter steps, paired arms identical')
...
def protocol_template():
    return dict(schema=SCHEMA,status='REVIEW_REQUIRED',training=copy.deepcopy(TRAINING),
        ...
        preflight_protocol_sha256=None,
        engineering_preflight_complete_sha256=None,
        ...
        analysis_sources_sha256={},selection_sha256=pre.protocol_template()['selection_sha256'],
        train_predictions=pre.protocol_template()['predictions'],development_predictions=dict(complete_sha256=None,manifest_sha256=None,predictions_sha256=None))
...
def validate_engineering(root,protocol,preflight_protocol,initial_head=None,initial_readout=None,initial_conditioner=None):
    ...
    require(not (root/'failed.json').exists() and sha(root/'complete.json')==
        protocol['engineering_preflight_complete_sha256'],'Wrong actual preflight endpoint')
    require(done['schema']==pre.SCHEMA and done['status']=='PASS_OBJECT_STATE_FORECAST_PREFLIGHT' and
    ...
    require(manifest['recipe']==pre.RECIPE and manifest['numerical_policy']==pre.NUMERICAL and
        manifest['sources']['protocol_sha256']==protocol['preflight_protocol_sha256'] and
    ...
    require(manifest['sources']['predictions']['complete_sha256']==protocol['train_predictions']['complete_sha256'],
        'Engineering/training predicted states differ')
    connection=summary['late_dual_loss_connectivity']
    for arm in ARMS:
        require(connection[arm]['after_completed_updates']==3,'Missing late preflight VJP')
        for family in ('occupancy','physical'):
            for target,prefix in (('future_head','transformer.'),('conditioner','state_mlp.')):
                stats=connection[arm]['loss_VJPs'][family][target]
                require(stats['norm']>0 and any(v is not None and v>0 for n,v in stats['parameter_norms'].items()
                    if n.startswith(prefix)),'Missing native/conditioner dual-loss preflight connection')
        intervention=summary['same_weights_velocity_interventions'][arm]
        require(intervention['empty_objects_native_path_exact'] and intervention['t0_self_parity'],
            'Missing empty/t0 engineering parity')
        for item in intervention['velocity_vs_zero'].values():
            require(item['max_abs_difference']>0 if arm=='V' else item['byte_equal'],
                'Missing V/G same-weights velocity intervention boundary')
    ...
    if initial_conditioner is not None:require(initial_conditioner==manifest['initial_conditioner_sha256'],'Fresh seed11 conditioner differs from preflight')
...
def load_contract(a):
    ...
    require(p['schema']==SCHEMA and p['status']=='FROZEN' and p['training']==TRAINING and
        p['numerical_policy']==pre.NUMERICAL,'Exact frozen formal V/G recipe required')
    ...
    require(sha(a.preflight_protocol)==p['preflight_protocol_sha256'],'Frozen preflight protocol differs')
    args=SimpleNamespace(**vars(a));args.protocol=a.preflight_protocol
    prior=read(a.preflight_protocol);args.max_seconds=prior['resources']['preflight']['max_seconds'];args.max_allocated_gib=prior['resources']['preflight']['max_allocated_gib']
    args.predictions=a.train_predictions;args.raw_metadata=a.raw_labels
    pp,contract,mods=pre.load_contract(args)
    expected=dict(pp['sources_sha256']);expected[Path(__file__).name]=sha(__file__)
    expected['object_state_forecast_development_inputs_v1.py']=DEV_INPUT_SOURCE_SHA
    ...
    require(set(p['analysis_sources_sha256'])=={'summarize_object_state_common_v1.py','summarize_object_state_physical_v1.py'}
    ...
    require(p['train_predictions']==pp['predictions'],'Formal prediction assets differ from real preflight')
...
def run(a,out,started,stop):
    ...
    predicted_train=input_loader.PredictionInputs(a.train_predictions,a.raw_labels,a.selection,p['train_predictions'],train)
    dev_input_loader=contract.import_bound('object_state_forecast_development_inputs_v1',p['sources_sha256'])
    predicted_dev=dev_input_loader.DevelopmentPredictionInputs(a.development_predictions,a.raw_labels,a.selection,dev,p['development_predictions'])
    import torch
    ...
    models={'V':model,'G':copy.deepcopy(model)};pre.seed_all(torch)
    readouts={'V':core.FutureStateMotionReadout().to(a.device)};readouts['G']=copy.deepcopy(readouts['V'])
    readout_initial=helper.state_digest(readouts['V'])
    conditioners={'V':condition_core.ObjectStateConditioner().to(a.device)};conditioners['G']=copy.deepcopy(conditioners['V'])
    conditioner_initial=helper.state_digest(conditioners['V'])
    engineering=validate_engineering(a.engineering_preflight,p,pp,head_initial,readout_initial,conditioner_initial)
    head_params={};readout_params={}
    for arm in ARMS:
        for param in models[arm].future_pred_head.parameters():param.requires_grad_(True)
        head_params[arm]=list(models[arm].future_pred_head.named_parameters());readout_params[arm]=[('readout.'+n,v) for n,v in readouts[arm].named_parameters()]+[('conditioner.'+n,v) for n,v in conditioners[arm].named_parameters()]
        require(sum(v.numel() for _,v in head_params[arm])==13274016 and sum(v.numel() for _,v in readout_params[arm])==745392+95872,'Parameter count differs')
        require(helper.state_digest(models[arm])==O_sha and helper.state_digest(readouts[arm])==readout_initial and helper.state_digest(conditioners[arm])==conditioner_initial,'Fresh arm initialization differs')
    ...
    sources=dict(protocol_sha256=sha(a.protocol),sources_sha256=p['sources_sha256'],preflight_protocol_sha256=p['preflight_protocol_sha256'],
        engineering_preflight_complete_sha256=p['engineering_preflight_complete_sha256'],connected_protocol_sha256=pre.CONNECTED_PROTOCOL_SHA,
        connected_complete_sha256=pre.CONNECTED_COMPLETE_SHA,D_checkpoint_sha256=pre.D_CHECKPOINT_SHA,
        ...
        train_predictions=predicted_train.receipt,development_predictions=predicted_dev.receipt)
    manifest=dict(schema=SCHEMA,mode='train',sources=sources,training=TRAINING,numerical_policy=pre.NUMERICAL,
        initial_O_state_sha256=O_sha,initial_future_head_sha256=head_initial,initial_readout_sha256=readout_initial,initial_conditioner_sha256=conditioner_initial,
    ...
    write(out/'manifest.json',manifest);write(out/'loaded_models.json',dict(O_full_state_sha256=O_sha,D=D_receipt,
        initial_future_head_sha256=head_initial,initial_readout_sha256=readout_initial,initial_conditioner_sha256=conditioner_initial,optimizer_restored=False))
    ...
    for arm,folder in folders.items():
        ...
        write(folder/'manifest.json',dict(schema=SCHEMA,arm=arm,common_manifest_sha256=sha(out/'manifest.json'),
            sources=sources,detach_physical_features=False,use_predicted_velocity=arm=='V',initial_future_head_sha256=head_initial,
            initial_readout_sha256=readout_initial,initial_conditioner_sha256=conditioner_initial,future_head_parameters=13274016,readout_parameters=745392,conditioner_parameters=95872))
    pre.seed_all(torch)
    for arm in ARMS:
    ...
    for pass_index,order in enumerate(orders):
        for ordinal in order:
            ...
            states,valid,state_receipt=predicted_train.get(ordinal,condition_core,a.device);state_digest=tree.tree_digest(states)
            ...
            for arm in ARMS:
                pre.restore_rng(torch,paired);budget()
                ...
                if updates==0:
                    ...
                    if arm=='V':initial_values=(native_digest,occ_audit,float(physical.detach()))
                    else:
                        ...
                        initial_comparison['physical_scalar_descriptive']=dict(V=initial_values[2],G=float(physical.detach()),
                            G_minus_V=float(physical.detach())-initial_values[2],same_D_fields_exact=True,same_sparse_source=True)
                if examples in (12,2044):
                    diagnostic={}
                    for family,scalar in (('occupancy',occupancy),('physical',physical)):
                        diagnostic[family]={}
                        for group,named in (('future_head',head_params[arm]),('conditioner',list(conditioners[arm].named_parameters()))):
                            vjp=torch.autograd.grad(scalar,[v for _,v in named],allow_unused=True,retain_graph=True)
                            diagnostic[family][group]=pre.gradient_stats(named,vjp);del vjp
                    event=dict(arm=arm,microstep_index=examples,after_completed_updates=updates,ordinal=ordinal,
                        ...
                        GMO_objects=int(valid.sum()),dual_loss_gradients=diagnostic,diagnostic_only=True,used_for_training_decisions=False)
                ...
                require(tree.tree_digest(sample['inputs'])==boundary and tree.tree_digest(states)==state_digest,'Input/state tokens mutated')
                batches[arm].append(dict(ordinal=ordinal,**common.identity(record),inputs_sha256=record['files']['inputs']['sha256'],
                    targets_sha256=record['files']['targets']['sha256'],sparse_label_sha256=descriptor['sha256'],input_tree_sha256=boundary,state_source=state_receipt,conditioner_hook=hook_audit,
            ...
            require(pre.rng_digest(ends['V'])==pre.rng_digest(ends['G']),'Paired RNG consumption differs')
            pre.restore_rng(torch,ends['V'])
            if initial_comparison is not None:
                for arm in ARMS:batches[arm][-1]['initial_occupancy_comparison']=initial_comparison
            del sample,base,paired,ends,states,valid
            ...
            for arm in ARMS:
                ...
                require(all(v is not None for n,v in rg['parameter_norms'].items() if n.startswith('readout.')),'Missing physical readout parameter graph')
                for name,norm in rg['parameter_norms'].items():had[arm][name]|=norm is not None and norm>0
                hn=float(torch.nn.utils.clip_grad_norm_([v for _,v in head_params[arm]],35.,error_if_nonfinite=True))
                rn=float(torch.nn.utils.clip_grad_norm_([v for _,v in readout_params[arm]],10.,error_if_nonfinite=True))
    ...
    final_conditioner={arm:helper.state_digest(m) for arm,m in conditioners.items()}
    optimizer_steps={};finals={}
    for arm in ARMS:
        optimizer_steps[arm]={}
        for name,opt in opts[arm].items():
            steps=[int(v['step'].item()) for v in opt.state.values()]
            require(steps and all(0<s<=512 for s in steps) and (name!='head' or all(s==512 for s in steps)),'Actual AdamW endpoint differs')
            optimizer_steps[arm][name]=dict(parameters_with_state=len(steps),steps=steps,all_steps512=all(s==512 for s in steps),
        ...
        payload=dict(schema=SCHEMA,status='FIXED_FINAL_512',arm=arm,future_pred_head=models[arm].future_pred_head.state_dict(),
            motion_readout=readouts[arm].state_dict(),conditioner=conditioners[arm].state_dict(),optimizers={name:opt.state_dict() for name,opt in opts[arm].items()},
            update=512,examples=2048,sample_orders=orders,sample_orders_sha256=helper.json_hash(orders),
            initial_future_head_sha256=head_initial,initial_readout_sha256=readout_initial,initial_conditioner_sha256=conditioner_initial,
            final_future_head_sha256=final_head[arm],final_readout_sha256=final_readout[arm],final_conditioner_sha256=final_conditioner[arm],
        ...
        finals[arm]=dict(file=str(path.relative_to(out)),sha256=sha(path),future_head_state_sha256=final_head[arm],readout_state_sha256=final_readout[arm],conditioner_state_sha256=final_conditioner[arm])
    require(optimizer_steps['V']==optimizer_steps['G'],'Matched parameter optimizer steps differ')
    ...
    baseline=copy.deepcopy(models['V']);baseline.future_pred_head.load_state_dict(initial_head_state,strict=True)
    ...
    for arm in ARMS:models[arm].eval();readouts[arm].eval();conditioners[arm].eval()
    ...
    with torch.no_grad():
        for ordinal,record in enumerate(dev):
            ...
            states,valid,state_receipt=predicted_dev.get(ordinal,condition_core,a.device);state_digest=tree.tree_digest(states);hook_audits={}
            for arm in ARMS:
                with condition_core.condition_future_head(models[arm].future_pred_head,conditioners[arm],states,valid,use_velocity=arm=='V') as hook_audits[arm]:
                    predictions[arm],features=core.capture_native_terminal(models[arm],sample,native,training=False)
                flows[arm]=readouts[arm](features,base,detach_features=False);del features
            # All O/V/G native predictions and D/V/G fields exist before labels.
            ...
            require(tree.tree_digest(sample['inputs'])==boundary and tree.tree_digest(states)==state_digest,'Development inputs/state tokens mutated')
            row=dict(ordinal=ordinal,**common.identity(record),hist_by_arm=hist,metrics_by_arm=metrics,
                ...
                t0_prediction_policy='model_specific_native',state_source=state_receipt,conditioner_hooks=hook_audits,
                t0_predictions_may_differ=True,t0_equality_gate_applied=False,GT_values_used_only_after_predictions=True,current_calibration_read_before_predictions=True,
            ...
            del sample,predictions,base,flows,gt,label,raw_bytes,raw,hist,metrics,states,valid
    ...
    require(all(helper.state_digest(models[a].future_pred_head)==final_head[a] and helper.state_digest(readouts[a])==final_readout[a] and helper.state_digest(conditioners[a])==final_conditioner[a]
    ...
    scores=common.summarize(rows,('O','V','G'),metric)
    physical_summary={arm:helper.epe_summary([r for r in objects if r['arm']==arm]) for arm in ('D','V','G')}
    summary=dict(schema=SCHEMA,status='COMPLETE_OBJECT_STATE_FORECAST_TRAINING',updates=512,examples=2048,evaluated_samples=200,
        scores=scores,physical=physical_summary,initial_future_head_sha256=head_initial,initial_readout_sha256=readout_initial,initial_conditioner_sha256=conditioner_initial,
        final_future_head_sha256=final_head,final_readout_sha256=final_readout,final_conditioner_sha256=final_conditioner,actual_optimizer_steps=optimizer_steps,
    ...
    for arm in ARMS:
        ...
        write(folders[arm]/'complete.json',dict(schema=SCHEMA,status=summary['status'],arm=arm,updates=512,examples=2048,
            evaluated_samples=200,final_future_head_sha256=final_head[arm],final_readout_sha256=final_readout[arm],final_conditioner_sha256=final_conditioner[arm],
...
def main(argv=None):
    ...
    for name in ('protocol','preflight-protocol','engineering-preflight','connected-protocol','training-run','config',
                 ...
                 'raw-labels','o-development-records','train-predictions','development-predictions','selection','out'):

...
SCHEMA = 'object-state-forecast-preflight-v1'
ARMS = ('V', 'G')
CONDITIONER_SHA = 'cdaf1ef70a2ca11ee7d027817046d767b5b341727694f07d24d9f9bee244102c'
...
RECIPE = dict(seed=11, updates=4, examples=16, accumulate=4, train_samples=512,
    planned_formal_updates=512, formal_authorized=False, conditioner_parameters=95680,
...
def load_contract(a):
    ...
    expected['object_state_conditioner_v1.py']=CONDITIONER_SHA
...
def run(a, out, started, stop):
    ...
    condition_core=contract.import_bound('object_state_conditioner_v1',p['sources_sha256'])
    ...
    for arm in ARMS:
        ...
        require(sum(v.numel() for _,v in head_params[arm])==13274016 and
                sum(v.numel() for _,v in readout_params[arm])==745392+95680, 'Trainable parameter count changed')

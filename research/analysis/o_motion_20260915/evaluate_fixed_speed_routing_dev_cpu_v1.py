"""Bounded CPU evaluation of all eight fixed speed routes on original dev200.

Input-only full-grid D/owner gates precede every sparse label payload read.
No fitting, policy selection, O model, or occupancy replay. CPU numerical parity
is checked at per-object EPE level against the frozen GPU diagnostic, not claimed
for every dense prediction. The independent train-anchor CPU probe is mandatory.
"""
import argparse
import hashlib
...
SCHEMA = 'fixed-speed-routing-dev-cpu-v1'
STATUS = 'COMPLETE_CPU_FIXED_SPEED_ROUTING_DEV_DIAGNOSTIC'
HORIZONS = (.5, 1., 1.5, 2.)
...
PROBE_SOURCE_SHA = 'f14b1d44e33e57cd533eb4360f4cd12d1f02e191d0a14bd864711918e277d1b8'
...
RESOURCES = dict(max_seconds=1800, outer_seconds=1860, cpu_threads=2, interop_threads=1, max_RSS_bytes=8*1024**3)
PROBE_TOLERANCES = dict(max_abs_displacement_m=1e-4, mean_abs_displacement_m=1e-6, max_gate_flips=0)
...
RECIPE = dict(samples=200, scenes=100, object_horizon_rows=16074, thresholds_mps=THRESHOLDS,
    ...
    numerical_reference='per-object XY/XYZ EPE only; no claim of all-point or bitwise CPU/GPU equivalence',
...
def protocol_template():
    """Parent fills own-source/probe hashes and source closure, then sets FROZEN."""
    return dict(schema=SCHEMA, status='REVIEW_REQUIRED', source_sha256='PENDING_ROOT_FREEZE',
        sources_sha256={}, scorer_source_sha256=SCORER_SHA, probe_source_sha256=PROBE_SOURCE_SHA,
        cpu_probe_complete_sha256='PENDING_PROBE_PASS', cpu_probe_report_sha256='PENDING_PROBE_PASS',
        ...
        recipe=RECIPE, resources=RESOURCES, probe_tolerances=PROBE_TOLERANCES, tolerances=TOLERANCES)
...
def authenticate_probe(root, protocol):
    for field in ('cpu_probe_complete_sha256', 'cpu_probe_report_sha256'):
        value = protocol[field]
        require(isinstance(value, str) and len(value) == 64 and all(c in '0123456789abcdef' for c in value),
                'Unfrozen CPU probe binding: '+field)
    root = Path(root)
    require(not (root/'failed.json').exists() and sha(root/'complete.json') == protocol['cpu_probe_complete_sha256'],
            'CPU probe missing, failed or changed')
    done = read(root/'complete.json'); check_ledger(root, done)
    require(sha(root/'report.json') == protocol['cpu_probe_report_sha256'], 'CPU probe report differs')
    report = read(root/'report.json')
    require(done['status'] == report['status'] == 'PASS_FIXED_TRAIN_ANCHOR_CPU_PROBE' and
            done['source_sha256'] == report['source_sha256'] == PROBE_SOURCE_SHA, 'CPU probe not authenticated PASS')
    require(report['tolerances'] == PROBE_TOLERANCES and report['threshold_mps'] == THRESHOLDS and
            report['loader']['checkpoint_sha256'] == CHECKPOINT_SHA, 'CPU probe recipe/checkpoint differs')
    require(report['numeric_pass'] is True and report['gate_pass'] is True and
            0 <= report['max_abs_displacement_m'] <= 1e-4 and 0 <= report['mean_abs_displacement_m'] <= 1e-6 and
            report['gate_flip_counts'] == [0]*8 and report['peak_RSS_bytes'] <= RESOURCES['max_RSS_bytes'],
            'CPU probe failed fixed numerical/resource limits')
    for name, expected in dict(training=False, optimizer_updates=0, development_read=False,
            target_arrays_read=False, full_grid_inference_before_reference_read=True,
            CUDA_initialized=False, model_state_unchanged=True).items():
        require(report[name] == expected, 'CPU probe scope differs: '+name)
    return report
...
def compare_objects(record, label, scored, refs, seen, comparisons, stream):
    for hi, horizon in enumerate(HORIZONS):
        for k, instance in enumerate(label['instance_tokens'].tolist()):
            ...
            for name in row:
                require(row[name] == ref[name], 'CPU/GPU reference metadata differs: '+name)
            row['CPU_baselines'] = {}; row['GPU_reference_arms'] = ref['arms']
            row['GPU_reference_parts'] = ref['parts']; row['CPU_minus_GPU_EPE_m'] = {}
            for ai, arm in enumerate(GPU_ARMS):
                row['CPU_baselines'][arm] = {}; row['CPU_minus_GPU_EPE_m'][arm] = {}
                tolerance = TOLERANCES['per_object_CV_abs_EPE_m' if ai == 1 else 'per_object_D_or_blend_abs_EPE_m']
                for di, dim in enumerate(('xy', '3d')):
                    cpu = float(s['baseline'][k, ai, di]); gpu = float(ref['arms'][arm]['epe_'+dim+'_m'])
                    delta = cpu-gpu; error = abs(delta)
                    require(np.isfinite(cpu) and np.isfinite(gpu), 'Nonfinite per-object EPE')
                    ...
                    require(error <= tolerance, 'Frozen CPU/GPU per-object tolerance failed: '+str(key)+' '+arm+'/'+dim+' '+str(error))
                    row['CPU_baselines'][arm]['epe_'+dim+'_m'] = cpu
                    row['CPU_minus_GPU_EPE_m'][arm][dim] = delta
...
def run(a, out, started, progress):
    require(os.environ.get('CUDA_VISIBLE_DEVICES') == '', 'Require CUDA_VISIBLE_DEVICES empty')
    ...
    for name in template:
        if name not in ('status', 'source_sha256', 'sources_sha256', 'cpu_probe_complete_sha256', 'cpu_probe_report_sha256'):
    ...
    probe = authenticate_probe(a.cpu_probe, p)
    ...
    runtime = dict(python=sys.version, torch=torch.__version__, numpy=np.__version__, device='cpu', threads=2,
                   interop_threads=1, mkldnn_enabled=torch.backends.mkldnn.enabled)
    for name in runtime:
        require(runtime[name] == probe['runtime'][name], 'Formal CPU runtime differs from passing probe: '+name)
    native._prepare_repo(a.repo)
    D, gate, loader = trainer.load_completed_arm(a.d_run/'runs/D', a.connected_protocol, helper, 'cpu')
    del gate
    require(loader['checkpoint_sha256'] == CHECKPOINT_SHA and not D.training and
            all(not q.requires_grad and q.device.type == 'cpu' for q in D.parameters()), 'D must be fixed CPU eval')
    ...
    manifest = dict(schema=SCHEMA, source_sha256=sha(__file__), protocol_sha256=sha(a.protocol), protocol=p,
        ...
        sparse_label_manifest_sha256=connected['labels']['manifest_sha256'], runtime=runtime,
        cpu_probe_complete_sha256=p['cpu_probe_complete_sha256'], cpu_probe_report_sha256=p['cpu_probe_report_sha256'],
        GPU_reference_complete_sha256=GPU_COMPLETE_SHA, GPU_reference_objects_sha256=GPU_OBJECTS_SHA,
        cpu_probe_scope='one train anchor; formal comparisons are per-object EPE, not all-point parity')
    ...
    def budget():
        ...
        require(rss_bytes() <= RESOURCES['max_RSS_bytes'], 'CPU peak RSS exceeds 8 GiB')
        require(not torch.cuda.is_initialized(), 'CUDA initialized during CPU evaluation')
    with (out/'objects.jsonl').open('x') as object_stream:
        for ordinal, record in enumerate(selected):
            ...
            tokens = helper.load_tokens(cache_root, record, native, 'cpu')
    ...
    require(sha(a.scorer) == SCORER_SHA and sha(a.protocol) == manifest['protocol_sha256'], 'Scorer/protocol changed during evaluation')
    ...
    summary = dict(schema=SCHEMA, status=STATUS, samples=200, scenes=100, source_points=source_points,
        ...
        CPU_GPU_per_object_EPE_comparison=comparisons, tolerances=TOLERANCES, all_reference_tolerances_passed=True,
        ...
        verification=dict(full_grid_gates_before_sparse_labels=True, all_gathered_gates_equal_scorer=True,
            ...
            occupancy_result=False, CUDA_initialized=False, D_state_unchanged=True,
            bitwise_CPU_GPU_parity_claimed=False, all_point_CPU_GPU_parity_claimed=False),
        synthetic_checks=qa, peak_RSS_bytes=rss_bytes(), seconds=time.monotonic()-started)
...
def main(argv=None):
    ...
    for name in ('protocol', 'connected-protocol', 'source-root', 'scorer', 'd-run', 'dev-cache', 'sparse-labels',
                 'geometry-cache', 'selection', 'gpu-reference', 'runtime-contract', 'cpu-probe', 'repo', 'out'):
    ...
    def stop(signum, frame):
        raise TimeoutError('Bounded CPU evaluation signal '+str(signum))

...
ROOT = N/'server_results/probes/future_observation_readout_train16_v3'
...
def main():
    ...
    assert manifest['source_sha256'] == sha(N/'future_observation_readout_probe_v3.py')
    ...
    for row in rows:
        ...
        for h, obs in enumerate(row['teacher_observations']):
            ...
            assert abs(obs['actual_dt_seconds']-.5*h) < .1

"""Object-state input to the native future-head query, without output masking.
...
class ObjectStateConditioner(nn.Module):
    ...
    def __init__(self):
        super().__init__()
        self.state_mlp = nn.Sequential(nn.Linear(STATE_DIM, LATENT_DIM), nn.SiLU(),
...
def condition_future_head(head, conditioner, states, valid, *, use_velocity):
    ...
    audit = dict(schema='object-state-conditioner-hook-v1', use_velocity=use_velocity,
